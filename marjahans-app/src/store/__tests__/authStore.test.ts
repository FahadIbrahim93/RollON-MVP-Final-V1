...
describe('authStore', () => {
  ...
  describe('login', () => {
    it('should login with valid admin credentials from API', async () => {
      const payload: MockLoginPayload = {
        user: { id: '1', name: 'Admin User', email: 'admin@marjahans.com', role: 'admin' },
        token: 'token-admin',
      };
      vi.spyOn(globalThis, 'fetch').mockResolvedValue(createFetchResponse(true, payload));

      const { login } = useAuthStore.getState();
      const result = await login('admin@marjahans.com', 'admin123');
  ...
  describe('logout', () => {
    it('should logout and clear user data', async () => {
      ...
      await login('admin@marjahans.com', 'admin123');
      expect(useAuthStore.getState().isAuthenticated).toBe(true);
