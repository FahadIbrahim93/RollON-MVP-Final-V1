...
describe('buildProductJsonLd', () => {
  it('builds product schema with absolute image and canonical URL', () => {
    ...
    expect((schema.image as string[])[0]).toBe('https://marjahans.com.bd/images/grinder.png');
    expect((schema.offers as { url: string }).url).toBe('https://marjahans.com.bd/product/cyber-grinder');
  });
...
describe('useDocumentSEO', () => {
  it('updates title, canonical, and social metadata', () => {
    ...
    expect(document.title).toBe(`Cyber Grinder | Marjahan's Bangladesh`);
    expect(document.querySelector('meta[property="og:type"]')?.getAttribute('content')).toBe('product');
    expect(document.querySelector('meta[property="og:image"]')?.getAttribute('content')).toBe(
      'https://marjahans.com.bd/images/grinder.png',
    );
    expect(document.querySelector('link[rel="canonical"]')?.getAttribute('href')).toBe(
      'https://marjahans.com.bd/product/cyber-grinder',
    );
  });
  ...
  it('injects JSON-LD payload and removes it on unmount', () => {
    ...
    const script = document.getElementById('marjahans-jsonld');
    expect(script).not.toBeNull();
    expect(script?.textContent).toContain('Cyber Grinder');

    view.unmount();
    expect(document.getElementById('marjahans-jsonld')).toBeNull();
  });
